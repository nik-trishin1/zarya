...
For a **large product hypothesis** (new segment/Job, value proposition, positioning, "should we build this", live-product diagnosis), also read `.cursor/skills/product-hypothesis/SKILL.md` and follow Next Move Theory — do not start from generic JTBD or from implementation tickets. Skip this on factory tickets, bugfixes, and already-approved specs.
...
**Process:** `process/ai-factory/` is the portable factory playbook. Project tickets stay under `apps/zarya-tg/docs/tickets/`. Large product hypotheses use Next Move Theory first (`process/next-move-theory/README.md`); NMT output is a hypothesis, not Definition of Ready.
...
## Cursor skills for product bets

| Skill | When |
|-------|------|
| `product-hypothesis` | Front door for a large bet — classifies and routes |
| `nmt-chat` | Conversational advisor; paste messy context |
| `nmt-diagnose` | Live product: risks, growth, next move |
| `nmt-market-research` | New idea: which segment + Jobs |
| `nmt-analyze-interviews` | Extract Jobs from interview files |
