...
`VITE_BOT_USERNAME` must match your Telegram bot handle (without `@`). Share links use `https://t.me/{username}?startapp=event_{id}&startApp=event_{id}&mode=fullscreen`. Default: `zarya_friends_bot`.
